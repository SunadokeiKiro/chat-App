...
- CSVの形式：1列目に質問タイプ、2列目に必須かどうか(R/NR)、3列目に質問文、4列目以降に選択肢を記述する。
...
## CSVフォーマットの具体例
- アンケートフォームを作成する際のCSVファイルのサンプルです。
- 1行目が1つの質問に対応します。

```csv
selection,R,イベントの満足度を教えてください,大変満足,満足,普通,不満,大変不満
star,R,セッションの分かりやすさはいかがでしたか？
input_long,NR,イベントで最も印象に残った点を教えてください。
multi_selection,NR,興味のある分野を教えてください（複数選択可）,Web開発,データサイエンス,AI/機械学習,インフラ,その他
input,R,お名前（ニックネーム可）
```

## CSVフォーマットの具体例が生成するフォーム
- **1行目:** 「イベントの満足度を教えてください」という必須の**単一選択**質問が作成され、「大変満足」から「大変不満」までの選択肢が設定されます。
- **2行目:** 「セッションの分かりやすさはいかがでしたか？」という必須の**星評価**（5段階）質問が作成されます。
- **3行目:** 「イベントで最も印象に残った点を教えてください。」という任意の**複数行テキスト入力**欄が作成されます。
- **4行目:** 「興味のある分野を教えてください（複数選択可）」という任意の**複数選択**質問が作成され、5つの選択肢が設定されます。
- **5行目:** 「お名前（ニックネーム可）」という必須の**一行テキスト入力**欄が作成されます。

# issue
- Ver 8.2.0とする。

- form集計結果ルームにて、表のCSVダウンロードボタンのいちを、集計結果、グラフタブの右に移行したい。

- form集計結果ルームにて、集計結果の表にて、回答者欄に学生か社会人かなど、ラベル情報も追記したい。

- form集計結果ルームにて、集計結果の表の横幅を、選択や複数選択の質問項目においてはもう少し横長にしたい(スマホ表示時)。今、横幅が1文字分など極端に少なくなってしまっている。選択肢の文字数分横幅を取るなどしてなんとかしたい。
